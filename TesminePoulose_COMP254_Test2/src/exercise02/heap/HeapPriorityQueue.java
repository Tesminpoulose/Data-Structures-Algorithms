...
public class HeapPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
  ...
  protected void upheap(int nodePos) {
    if (nodePos <= 0) {
      return;
    }
    int parentNode = parent(nodePos);
    if (compare(heap.get(nodePos), heap.get(parentNode)) >= 0) {
      return;
    }
    swap(nodePos, parentNode);
    upheap(parentNode);
  }
  ...
  public void printHeap() {
    for (int i = 0; i < this.size(); i++)
      System.out.println("(" + this.heap.get(i).getKey() +
              ", " + this.heap.get(i).getValue() + ")");

    //get the minimum
    System.out.println("Key Value: " +this.min().getKey());
